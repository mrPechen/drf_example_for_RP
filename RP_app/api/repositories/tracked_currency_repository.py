...
from RP_app.api.models import Currency
...
class TrackedCurrencyRepository:
    ...
    """
    Запрос в БД на удаление отслеживаемого объекта.
    """

    def delete_tracked_currency(self, email: str, currency_id: int):
        user = self.user_model.objects.get(email=email)
        result = self.tracked_currency_model.objects.get(user=user, currency=currency_id)
        return result.delete()

    """
    Запрос в БД на изменение отслеживаемого значения объекта.
    """

    def change_tracked_currency_threshold(self, email: str, currency_id: int, threshold: float):
        user = self.user_model.objects.get(email=email)
        result = self.tracked_currency_model.objects.filter(user=user, currency=currency_id).first()
        result.update(threshold=threshold)
        return result

        # return {"success": f"{result.currency.charcode}`s threshold was updated to {threshold}"}
